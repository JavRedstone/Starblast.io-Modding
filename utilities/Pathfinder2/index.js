...
installed_modes = [
  ...
  },
  {
    ...
    asteroidsDensityModifier: function(t, e) {
      var i, s;
      return i = Math.sqrt(t * t + e * e), s = .5 * Math.sqrt(2), i = Math.abs(i - s), i < .15 ? 0 : 1
    }
  }
]
var size = sizeV, edge = size/2, t = new llOO1(seedV, edge, installed_modes[modeV]), map = [...new Array(sizeV)].map(i=> new Array(sizeV).fill(0)), f = {};
...
fs.writeFileSync('./map.json', JSON.stringify(map_json));