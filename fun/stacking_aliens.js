...
this.tick = function(game) {
  if (game.step > 500 && game.step % 5 === 0) {
    ...
    if (game.aliens.length < amount) game.addAlien({code: arr[Math.floor(Math.random() * arr.length)], level: Math.floor(Math.random() * 2), crystal_drop: 200});
    for (let i = 0; i < game.aliens.length; i++) {
      var alien = game.aliens[i];
      if (i > 0) alien.set({x: game.aliens[0].x, y: game.aliens[0].y, vx: 0, vy: 0});
      alien.set({rate: 5, damage: 10, laser_speed: 250});
    }
    
    if (game.step % 1200 === 0) {
      game.aliens[0].set({x:(Math.round(Math.random()) == 0 ? 1 : -1) * Math.random() * 60 * 5, y: (Math.round(Math.random()) == 0 ? 1 : -1) * Math.random() * 60 * 5});
      game.addAsteroid({x:(Math.round(Math.random()) == 0 ? 1 : -1) * Math.random() * 60 * 5, y: (Math.round(Math.random()) == 0 ? 1 : -1) * Math.random() * 60 * 5});
    }
  }
}
